//! Handling for `textDocument/documentSymbol` requests.
...
use crate::{core::Source, util, UnitResult};

pub(crate) fn handle(conn: &Connection, src: &Source, req_id: RequestId) -> UnitResult {
	let green = src.green.as_ref().unwrap();
	...
	for child in cursor.children() {
		...
		match toplevel {
			ast::TopLevel::ClassDef(classdef) => {
				if let Some(docsym) = doc_symbol_class(src, classdef) {
					docsyms.push(docsym);
				}
			}
			ast::TopLevel::ClassExtend(classext) => {
				if let Some(docsym) = doc_symbol_classext(src, classext) {
					docsyms.push(docsym);
				}
			}
			ast::TopLevel::ConstDef(constdef) => {
				if let Some(docsym) = doc_symbol_const(src, constdef) {
					docsyms.push(docsym);
				}
			}
			ast::TopLevel::EnumDef(enumdef) => {
				if let Some(docsym) = doc_symbol_enum(src, enumdef) {
					docsyms.push(docsym);
				}
			}
			ast::TopLevel::MixinClassDef(mixindef) => {
				if let Some(docsym) = doc_symbol_mixin(src, mixindef) {
					docsyms.push(docsym);
				}
			}
			ast::TopLevel::StructDef(structdef) => {
				if let Some(docsym) = doc_symbol_struct(src, structdef) {
					docsyms.push(docsym);
				}
			}
			ast::TopLevel::StructExtend(structext) => {
				if let Some(docsym) = doc_symbol_structext(src, structext) {
					docsyms.push(docsym);
				}
			}
...
fn doc_symbol_class(src: &Source, classdef: ast::ClassDef) -> Option<DocumentSymbol> {
	...
	let ret = DocumentSymbol {
		...
		tags: None,
		deprecated: None,
		range: src.make_range(classdef.syntax().text_range()),
		selection_range: src.make_range(name.text_range()),
		children: {
			let mut children = vec![];

			for innard in classdef.innards() {
				doc_symbol_class_innard(src, &mut children, innard);
			}
...
#[must_use]
fn doc_symbol_classext(src: &Source, classext: ast::ClassExtend) -> Option<DocumentSymbol> {
	...
	let ret = DocumentSymbol {
		...
		tags: None,
		deprecated: None,
		range: src.make_range(classext.syntax().text_range()),
		selection_range: src.make_range(name.text_range()),
		children: {
			let mut children = vec![];
...
fn doc_symbol_class_innard(
	...
	match innard {
		...
		ast::ClassInnard::StaticConst(sconst) => {
			if let Some(docsym) = doc_symbol_staticconst(src, sconst) {
				docsyms.push(docsym);
			}
		}
		ast::ClassInnard::Function(fndecl) => {
			if let Some(docsym) = doc_symbol_function(src, fndecl) {
				docsyms.push(docsym);
			}
		}
		ast::ClassInnard::Field(field) => {
			if let Some(dsyms) = doc_symbol_field(src, field) {
				for docsym in dsyms {
					docsyms.push(docsym);
				}
			}
		...
		ast::ClassInnard::Property(propdef) => {
			...
			docsyms.push(DocumentSymbol {
				...
				tags: None,
				deprecated: None,
				range: src.make_range(propdef.syntax().text_range()),
				selection_range: src.make_range(name.text_range()),
				children: None,
			});
		}
		ast::ClassInnard::Flag(flagdef) => {
			...
			docsyms.push(DocumentSymbol {
				...
				tags: None,
				deprecated: None,
				range: src.make_range(flagdef.syntax().text_range()),
				selection_range: src.make_range(name.text_range()),
				children: None,
			});
		}
			...
			for innard in states.innards() {
				match innard {
					ast::StatesInnard::Label(label) => {
						let range = src.make_range(label.syntax().text_range());
...
#[must_use]
fn doc_symbol_const(src: &Source, constdef: ast::ConstDef) -> Option<DocumentSymbol> {
	...
	let ret = DocumentSymbol {
		...
		tags: None,
		deprecated: None,
		range: src.make_range(constdef.syntax().text_range()),
		selection_range: src.make_range(name.text_range()),
		children: None,
	};
...
#[must_use]
fn doc_symbol_enum(src: &Source, enumdef: ast::EnumDef) -> Option<DocumentSymbol> {
		...
		children: {
			...
			for variant in enumdef.variants() {
				...
				children.push(DocumentSymbol {
					...
					tags: None,
					deprecated: None,
					range: src.make_range(variant.syntax().text_range()),
					selection_range: src.make_range(vname.text_range()),
					children: None,
				});
			}
...
fn doc_symbol_field(
	src: &Source,
	field: ast::FieldDecl,
) -> Option<impl Iterator<Item = DocumentSymbol> + '_> {
	...
	let ret = field.names().map(move |varname| {
		...
		DocumentSymbol {
			...
			tags: None,
			deprecated: None,
			range: src.make_range(field.syntax().text_range()),
			selection_range: src.make_range(varname.syntax().text_range()),
			children: None,
		}
	});
...
#[must_use]
fn doc_symbol_function(src: &Source, fndecl: ast::FunctionDecl) -> Option<DocumentSymbol> {
	...
	let ret = DocumentSymbol {
		...
		deprecated: None,
		range: src.make_range(fndecl.syntax().text_range()),
		selection_range: src.make_range(name.text_range()),
		children: None,
	};
...
#[must_use]
fn doc_symbol_mixin(src: &Source, mixindef: ast::MixinClassDef) -> Option<DocumentSymbol> {
	...
	let ret = DocumentSymbol {
		...
		tags: None,
		deprecated: None,
		range: src.make_range(mixindef.syntax().text_range()),
		selection_range: src.make_range(name.text_range()),
		children: {
			let mut children = vec![];
...
#[must_use]
fn doc_symbol_staticconst(src: &Source, sconst: ast::StaticConstStat) -> Option<DocumentSymbol> {
	...
	let ret = DocumentSymbol {
		...
		tags: None,
		deprecated: None,
		range: src.make_range(sconst.syntax().text_range()),
		selection_range: src.make_range(name.text_range()),
		children: None,
	};
...
#[must_use]
fn doc_symbol_struct(src: &Source, structdef: ast::StructDef) -> Option<DocumentSymbol> {
	...
	let ret = DocumentSymbol {
		...
		tags: None,
		deprecated: None,
		range: src.make_range(structdef.syntax().text_range()),
		selection_range: src.make_range(name.text_range()),
		children: Some({
			let mut children = vec![];

			for innard in structdef.innards() {
				doc_symbol_struct_innard(src, &mut children, innard);
			}
...
#[must_use]
fn doc_symbol_structext(src: &Source, structext: ast::StructExtend) -> Option<DocumentSymbol> {
	...
	let ret = DocumentSymbol {
		...
		tags: None,
		deprecated: None,
		range: src.make_range(structext.syntax().text_range()),
		selection_range: src.make_range(name.text_range()),
		children: {
			let mut children = vec![];
